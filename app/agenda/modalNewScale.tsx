...
export default function ModalNewScale({ onClose, onSuccess }: ModalNewScaleProps) {
    const [form, setForm] = useState({
        ...
        event: '',
        responsible: '',
        ministriesSelected: [] as string[], // array de ministry_id
        // Ex.: { [ministryId]: memberIds[] }
    ...
    function toggleMinistry(ministryId: string) {
        setForm((prev) => {
            ...
            let nextMinistries = exists
                ? prev.ministriesSelected.filter((id) => id !== ministryId)
                : [...prev.ministriesSelected, ministryId];

            // Se removeu o ministério, limpa membros selecionados desse ministério
    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        try {
            ...
            await createScale({
                ...
                assignments, // não aparecerá na tabela; será usado em "ver detalhes"
            });
                            ...
                            {ministries.map((m) => {
                                ...
                                return (
                                    <button
                                        ...
                                        className={`px-3 py-1 rounded-full text-xs font-medium border transition ${selected
                                ...
                                {ministriesSelectedObjects.map((min) => {
                                    ...
                                    return (
                                        <div key={min.id} className="mb-3">
                                            ...
                                            <div className="flex flex-wrap gap-2">
                                                {list.length === 0 && (
                                                    <span className="text-xs text-gray-400">Nenhum membro encontrado</span>
                                                )}
                                                {list.map((mem) => {
                                                    const selected = (form.selectedByMinistry[min.id] || []).includes(mem.id);
                                                    return (
                                                        <button
                                                            key={mem.id}
                                                            type="button"
